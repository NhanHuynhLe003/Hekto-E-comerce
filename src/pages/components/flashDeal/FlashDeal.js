...
export default function FlashDeal({ productImg, priceDiscounted=29, title="Sky Monkey 2020", cateText="Camera,photo & video", price="39.00", codeProduct="E426L" }) {
  return (
    <div className={style.cardWrapper}>
      <div className={style.card}>
        ...
        <div className={style.controller}>
            <button className={style.addToCart}><Image src={shoppingCart} alt="shoppingIcon"/></button>
            <button className={style.addToLike}><Image src={heartIcon} alt="heartIcon"/></button>
            <button className={style.zoom}><Image src={zoomIcon} alt="zoomIcon"/></button>
        </div>
      </div>
    </div>
  );
