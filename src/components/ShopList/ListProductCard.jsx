...
import style from "./listProductCard.module.scss";
...
export default function ListProductCard() {
  const methodSelect = useSelector((state) => state.app.shopList.sortBy);
  const searchResSelector = useSelector(
    (state) => state.app.shopList.searchResult
  );
  ...
  return (
    <div
      className={clsx({
        [style.listCard]: view === 2,
        [style.listCardViewFirst]: view === 1,
      })}
    >
      {listProduct ? (
        listProduct
